...
public:
	...
	// PS3EyeVCam patch: wake any blocked Dequeue and make it fail fast. Called
	// when transfers close (camera stop / device unplug) so a consumer thread
	// never hangs on a queue that will produce no more frames.
	void Abort()
	{
		{
	...
	// PS3EyeVCam patch: bounded wait + abort awareness; returns false when no
	// frame was produced (timeout or aborted) instead of blocking forever.
	bool Dequeue(uint8_t* new_frame, int frame_width, int frame_height, PS3EYECam::EOutputFormat outputFormat, bool flip_v, uint32_t timeoutMs)
	{
		std::unique_lock<std::mutex> lock(mutex);

		// If there is no data in the buffer, wait until data becomes available
		if (!empty_condition.wait_for(lock, std::chrono::milliseconds(timeoutMs),
		                              [this] () { return available != 0 || aborted; }))
			return false;
		if (aborted || available == 0)
			return false;
...
public:
	...
	~URBDesc()
	{
		...
		close_transfers();
		// PS3EyeVCam patch: queue deletion deferred out of close_transfers (a
		// consumer may still be inside Dequeue when transfers close); by the
		// time the URBDesc dies no consumer can reference it anymore.
		delete frame_queue;
		frame_queue = NULL;
	}
	...
	{
		// Initialize the frame queue (deleting any queue left from a previous
		// streaming session, see ~URBDesc)
	...
	{
		// PS3EyeVCam patch: unblock any consumer waiting in Dequeue before
		// (and regardless of) transfer teardown.
		if (frame_queue != NULL)
			frame_queue->Abort();
		...
		transfer_buffer = NULL;
		// PS3EyeVCam patch: frame_queue intentionally NOT deleted here -- see
...
bool PS3EYECam::getFrame(uint8_t* frame, uint32_t timeoutMs)
{
	FrameQueue* queue = urb ? urb->frame_queue : NULL;
	if (queue == NULL)
		return false;
	return queue->Dequeue(frame, frame_width, frame_height, frame_output_format, flip_v, timeoutMs);
}
